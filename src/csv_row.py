...
def csvrow_deleteRows(source, rowNumbers):
    ...
    try:
        ...
        rowNumbers = np.unique(rowNumbers).tolist()
...
"""
機能   :  DataFrameの指定した列が、指定したkeyと”等しくない”レコードを検索
          keyと一致したレコードの指定した列の値を、指定したvalueで置き換える
引数   :
            DataFrame   :   DataFrame形式のデータ
            int        :   検索する対象の列番号
...
戻り値  :
            ...
            DataFrame   :   setValueのDataFrame形式のデータ
"""
def csvrow_setValueInRowsSearchedByExceptionKey(source, columnNumber, exceptionKey, targetColumns, value):
    result = const.RESULT_COMPLETE      # ステータス
    msg = const.MSG_COMPLETE            # メッセージ
    newData = pandas.DataFrame()        # setValueのDataFrame形式のデータ
    ...
    try:
        ...
        # [key]が[string]のデータ型以外の場合
        # [value]が[string]のデータ型以外の場合
        if type(exceptionKey) is not str  or type(value) is not str:
            raise Exception
        exceptionKey = exceptionKey.strip()
        ...
        recordSearch = source[source.iloc[:, columnNumber - 1] != exceptionKey].index
        # keyと一致する列が一つもなかった
        if (len(recordSearch) == 0):
            result = const.RESULT_ERR
            msg = const.MSG_ERR_KEY_NOT_FOUND.format(exceptionKey, columnNumber)
        else:
            # 処理が問題なく完了した
            targetColumnsUnique = list(np.asarray(targetColumnsUnique) - 1)
            source.iloc[recordSearch, targetColumnsUnique] = value
